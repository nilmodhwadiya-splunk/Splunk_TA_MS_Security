    ...
    def stream_events(self, inputs: smi.InputDefinition, ew: smi.EventWriter) -> None:
        ...
        for input_name, input_item in inputs.inputs.items():
            ...
            if access_token:
                ...
                simulations = get_atp_alerts_odata(
                    logger,
                    session_key,
                    access_token,
                    simulation_url,
                ...
                simulation_ingested = 0
                for simulation in simulations:
                    simulation_id = simulation.get("id")
                    required(
                        simulation_id,
                        "Field denoting simulation id not found in the API response",
                    )
                    simulation_report_overview = get_simulation_report(
                        logger,
                        session_key,
                        access_token,
                        urls.simulation_report.format(simulation_id),
                        user_agent=f"M365DPartner-Splunk-MicrosoftSecurityAddOn/{current_version}",
                    )
                    simulation.update(simulation_report_overview)
                    event = smi.Event(
                        data=json.dumps(simulation, ensure_ascii=False, default=str),
                        index=input_item["index"],
                        source=f"microsoft_defender_endpoint_simulations:{input_item['tenant_id']}",
                        sourcetype=input_item["sourcetype"],
                    )
                    ew.write_event(event)

                    completion_date_time = simulation.get("completionDateTime", None)
                    required(
                        completion_date_time,
                        "Field denoting simulation completionDateTime not found in the API response",
                    )

                    if completion_date_time > max_simulation_date:
                        max_simulation_date = completion_date_time

                    simulation_ingested = simulation_ingested + 1

                logger.info(
                    f"Simulations ingested - simulation_count={simulation_ingested}"
                )
                checkpoint_handler(
                    logger,
                    session_key,
